...
const EditForm = ({ ChallanData }) => {
  ...
  //const {Bill : Bills} = Digit.Hooks.(ChallanData[0].challanNo, tenantId, ChallanData[0].businessService);
  ...
  debugger;
  let billdetailsDV = fetchBillData ? fetchBillData.Bill[0].billDetails[0].billAccountDetails : [];
  console.log("default value");
  console.log(billdetailsDV);

  let defaultval = {};
  billdetailsDV.map((bd) => {
    defaultval[`${bd.taxHeadCode.split(".")[1]}`] = `${bd.amount}`;
  });

  let defaultvalfinal = {};
  fetchBillData ? (defaultvalfinal[`${fetchBillData.Bill[0].businessService.split(".")[0]}`] = defaultval) : {};
  console.log(defaultvalfinal);
  debugger;
  const defaultValues = {
    formData: {
      citizen: {
    ...
    },
    {
      ...
      body: [
        ...
        },
        {
          isMandatory: false,
          type: "custom",
          populators: <TaxForm ref={childRef} data={TaxHeadMasterFields} defaultValues={defaultvalfinal} fetchBillData={fetchBillData} />,
        },
        {
